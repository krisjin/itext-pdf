	...
	public static URL getURL(String resourceLocation, ClassLoader loader) throws RuntimeException {
		...
		if (file.exists()) {
			try {
				...
				try {
					...
					throw new RuntimeException("The [resourceLocation=" + resourceLocation
							+ "] is neither a URL not a well-formed file path.", ex2);
				}
			}
		}
		URL url = null;
		...
		if (url == null) {
			...
			if (url == null) {
				...
				if (url == null) {
					throw new RuntimeException("Class path resource [" + resourceLocation
